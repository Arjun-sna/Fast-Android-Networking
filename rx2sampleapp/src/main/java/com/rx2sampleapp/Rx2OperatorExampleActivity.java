...
public class Rx2OperatorExampleActivity extends AppCompatActivity {
   ...
   *********************************/
    ...
    */
  private Single<List<User>> getCricketFansObservable() {
  ...
  */
  private Single<List<User>> getFootballFansObservable() {
  ...
  */
  private void findUsersWhoLovesBoth() {
    ...
    Single.zip(getCricketFansObservable(), getFootballFansObservable(),
  ...
  private Single<List<User>> getAllMyFriendsObservable() {
    return Rx2AndroidNetworking.get(
        "https://fierce-cove-29863.herokuapp.com/getAllFriends/{userId}")
        .addPathParameter("userId", "1")
        .build()
        .getObjectListObservable(User.class);
  }
  ...
  public void flatMapAndFilter(View view) {
    getAllMyFriendsObservable().toObservable()
        .flatMap(
            new Function<List<User>, ObservableSource<User>>() { // flatMap - to return users one by one
              @Override
              public ObservableSource<User> apply(List<User> usersList) throws Exception {
                return Observable.fromIterable(
                    usersList); // returning user one by one from usersList.
              }
            })
  ...
  public void take(View view) {
    getUserListObservable().toObservable()
        .flatMap(
            new Function<List<User>, ObservableSource<User>>() { // flatMap - to return users one by one
              @Override
              public ObservableSource<User> apply(List<User> usersList) throws Exception {
    ...
    getUserListObservable().toObservable()
        ...
        .flatMap(new Function<User, ObservableSource<UserDetail>>() {
          ...
          public ObservableSource<UserDetail> apply(User user) throws Exception {
            ...
            // for that userId
            return getUserDetailObservable(user.id).toObservable();
          }
        })
        .subscribeOn(Schedulers.newThread())
        .observeOn(AndroidSchedulers.mainThread())
  ...
  private Single<List<User>> getUserListObservable() {
    return Rx2AndroidNetworking.get(
  ...
  private Single<UserDetail> getUserDetailObservable(long id) {
    return Rx2AndroidNetworking.get(
        "https://fierce-cove-29863.herokuapp.com/getAnUserDetail/{userId}")
    ...
    getUserListObservable().toObservable()
        ...
        .flatMap(new Function<User, ObservableSource<Pair<UserDetail, User>>>() {
          ...
          public ObservableSource<Pair<UserDetail, User>> apply(User user) throws Exception {
            ...
            return Observable.zip(getUserDetailObservable(user.id).toObservable(),
